                    ...
                    void *buf, size_t maxlen)
{
    ...
    while ((!rd->res_lookif || !rd->ep_lookif) || (parsed_len != _result_buf_len)) {
        ssize_t ret = clif_decode_link(&lookif,
                          attrs + attrs_used,
                          (sizeof(attrs) / sizeof(clif_attr_t)) - attrs_used,
                    ...
                    cord_lc_filter_t *filters, void* buf, size_t maxlen)
{
    ...
    new_filters.len = sizeof(new_array) / sizeof(clif_attr_t);
    ...
    /* parse the result */
    retval = clif_decode_link(&resource->link, resource->attrs,
                              resource->max_attrs, buf, retval);
    if (retval < 0) {
                   ...
                   cord_lc_filter_t *filters, void* buf, size_t maxlen)
{
    ...
    /* parse the result */
    retval = clif_decode_link(&endpoint->link, endpoint->attrs,
                              endpoint->max_attrs, buf, retval);
    if (retval < 0) {
